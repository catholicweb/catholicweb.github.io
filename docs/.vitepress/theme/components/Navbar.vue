    ...
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <div class="flex items-center">
          <a :href="currentLang?.path || '/'" class="text-xl font-bold text-gray-900 hover:text-accent transition-colors">
            {{ site.title }}
          </a>
        </div>
        ...
        <div class="hidden lg:flex items-center space-x-1 font-medium text-md">
          <template v-for="item in nav" :key="item.text">
            ...
            <div v-else class="relative group">
              ...
              <!-- Dropdown Menu -->
              <div class="absolute left-0 mt-0 w-96 bg-white rounded-sm shadow-lg opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all z-50 hover:text-accent">
        ...
        <div class="flex items-center space-x-2">
          ...
          <div v-if="languages.length > 1" class="relative group">
            ...
            <div class="absolute right-0 w-36 bg-white opacity-0 group-hover:opacity-100 invisible group-hover:visible group-focus-within:visible group-focus-within:opacity-100 transition-all z-50 hover:text-accent">
              <a v-for="lang in languages" :key="lang.code" @click.prevent="switchLanguage(lang)" class="block px-3 py-2 rounded-sm hover:bg-gray-100 cursor-pointer transition-colors" :class="lang.code === currentLang.code ? 'text-accent' : 'text-gray-700 '">
                {{ lang.label }}
              </a>
            </div>
          </div>
          ...
          <div v-if="themes.length > 0" class="relative group">
            ...
            <div class="absolute right-0 w-64 bg-white opacity-0 group-hover:opacity-100 invisible group-hover:visible group-focus-within:visible group-focus-within:opacity-100 transition-all z-50">
              <div v-for="themeItem in themes" :key="themeItem.id" @click.prevent="switchTheme(themeItem.id)" class="flex items-start gap-3 px-3 py-2 rounded-sm hover:bg-gray-100 cursor-pointer transition-colors" :class="themeItem.id === currentTheme ? 'text-accent' : 'text-gray-700 '">
